...
public class EncryptionUtil {
    ...
    public byte[] determineCipherMode(String urlParameter, int mode, Cipher cipher) {
        try {
            return switch (mode) {
                case ENCRYPT_MODE -> cipher.doFinal(urlParameter.getBytes());
                case DECRYPT_MODE -> cipher.doFinal(Base64.getDecoder().decode(urlParameter));
    ...
    public String encrypt(String surveyId) {
        try {
            ...
            return Base64.getUrlEncoder().encodeToString(determineCipherMode(surveyId, ENCRYPT_MODE, cipher));
        } catch (Exception e) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "해당 링크는 잘못된 링크입니다.", e);
        }
